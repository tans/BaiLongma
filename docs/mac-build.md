...
本文档面向 LiloAvatar 的开发者和维护者，用于构建、检查和排查 macOS 桌面安装包问题。
...
```text
/Users/xyf/Library/Application Support/LiloAvatar
```
...
```text
dist/LiloAvatar-2.1.436-mac-arm64.dmg
dist/LiloAvatar-2.1.436-mac-x64.dmg
```
...
LiloAvatar 当前为 Intel 和 Apple Silicon 分别构建 macOS DMG。
...
不要通过删除用户数据来修依赖问题。依赖状态位于仓库工作区，主要是 `node_modules/`，不在 LiloAvatar 用户数据目录中。
...
```text
~/Library/Application Support/LiloAvatar
```
...
```bash
hdiutil attach -readonly -nobrowse dist/LiloAvatar-2.1.436-mac-arm64.dmg
lipo -archs "/Volumes/<mounted-volume>/LiloAvatar.app/Contents/MacOS/LiloAvatar"
lipo -archs "/Volumes/<mounted-volume>/LiloAvatar.app/Contents/Resources/app.asar.unpacked/build/native-speech-recognizer"
lipo -archs "/Volumes/<mounted-volume>/LiloAvatar.app/Contents/Resources/app.asar.unpacked/node_modules/better-sqlite3/build/Release/better_sqlite3.node"
...
```text
"LiloAvatar.app" cannot be opened because Apple cannot check it for malicious software.
```

或：

```text
"LiloAvatar.app" is damaged and can't be opened. You should move it to the Trash.
```
...
```bash
spctl --assess --verbose "/Applications/LiloAvatar.app"
codesign --verify --deep --strict --verbose=2 "/Applications/LiloAvatar.app"
xattr -lr "/Applications/LiloAvatar.app" | rg com.apple.quarantine
```

移除 quarantine 只是本地测试绕过手段，不是发布流程：

```bash
xattr -dr com.apple.quarantine "/Applications/LiloAvatar.app"
```
...
```bash
tail -n 200 "$HOME/Library/Application Support/LiloAvatar/logs/bailongma.log"
```
...
如果本地测试时权限弹窗不再出现，只重置相关的 macOS 隐私权限条目。不要为了修复 TCC 权限状态而清空 LiloAvatar 用户数据。
