...
这份文档用于发给远程协作者，让对方在 macOS 机器上构建 LiloAvatar 安装包。更完整的排障说明见 `docs/mac-build.md`。
...
```text
LiloAvatar-<version>-mac-arm64.dmg
LiloAvatar-<version>-mac-x64.dmg
*.blockmap
latest-mac.yml
```
...
该命令会检查 DMG、`LiloAvatar.app`、app 可执行文件架构、macOS 语音 helper 架构和 `better-sqlite3` 原生模块架构。
...
- 不要删除用户数据目录：`~/Library/Application Support/LiloAvatar`。
