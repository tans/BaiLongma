const CAPABILITY_DEMO_CANDIDATE_RE = /(?:你|LiloAvatar|bailongma|agent|ai|小白龙).{0,10}(?:能|会|可以|能够).{0,10}(?:做|干|帮|完成).{0,10}(?:什么|啥|哪些事|哪些事情)|(?:你|LiloAvatar|bailongma|agent|ai|小白龙).{0,10}(?:有什么|有哪些).{0,10}(?:能力|功能|本事)|(?:能力|功能).{0,8}(?:展示|演示|秀一下|介绍|showcase|demo)|(?:展示|演示|秀一下).{0,8}(?:能力|功能)|what can you do|show(?: me)?(?: your)? capabilit/i
...
- If the user's intent is asking what you/LiloAvatar can do, or explicitly requests a capability/function demo, showcase, or self-introduction through abilities, you MUST call capability_demo instead of answering with plain text.
