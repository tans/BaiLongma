// audio-output.js — LiloAvatar语音输出设备路由
//
// 背景 / 为什么需要这个模块：
//   LiloAvatar是"会说话"的助手，TTS 默认走 `new Audio()`，也就是听天由命跟着系统默认播放设备走。
//   而大众用户的机器上经常装着虚拟声卡（Steam 串流 / NVIDIA Virtual / VB-Audio / Pico VR 等），
//   或者拔掉耳机后 Windows 没有正确回退到内置扬声器 —— 系统默认就卡在一个"哑巴"虚拟设备上。
//   结果：LiloAvatar在说话，但用户什么都听不到，而且双方都不知道为什么。对语音产品这是致命的静默失败。
//
...
// 关键：LiloAvatar TTS 只要 AudioContext 在运行，就会经 createMediaElementSource 走 Web Audio，
